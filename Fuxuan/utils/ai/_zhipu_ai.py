...
from ._data_model import Message, FUXUAN_META_PATH, START_SYS_PROMPT


class Session:
    """AI角色扮演会话类"""
    def __init__(self, model_name: str, api_key: str, character_meta_file: str | Path = FUXUAN_META_PATH, start_system_prompt: str = START_SYS_PROMPT, stream: bool = False):
    ...
    def _send(self, usr_message: str, output_to_console: bool = True, *args, **kwargs) -> str:
        ...
        if self.stream:
            ...
            if output_to_console:
                self._write_to_stdout(response.choices[0].message.content) # type: ignore
...
if __name__ == '__main__':
    session = Session("glm-4v-flash", "your_api_key", stream=True)
