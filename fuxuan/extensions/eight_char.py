...
from ..utils.const import TIANGAN , DIZHI
...
class CalculateEightChar:
    ...
    def _validate_date(date: datetime) -> None:
        ...
        if not isinstance(date, datetime):
            raise ArgumentRequired("日期参数类型错误，应为datetime.datetime类型")
        if not date.year:
            raise ArgumentRequired("日期参数错误，年份不能为空")
        if not date.month:
            raise ArgumentRequired("日期参数错误，月份不能为空")
        if not date.day:
            raise ArgumentRequired("日期参数错误，日不能为空")
        if not date.hour:
            raise ArgumentRequired("日期参数错误，时不能为空")

    @staticmethod
    def get_eight_char(date: datetime) -> EightChar:
        """计算生辰八字"""
        CalculateEightChar._validate_date(date)
        ...
        return EightChar(year, month, day, hour, birthday=date)
